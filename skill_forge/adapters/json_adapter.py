"""JSON adapter - formats JSON and highlights capability fields with value previews."""
...
def to_text(parsed_asset: dict) -> str:
    """Return formatted JSON text with capability highlights and value previews."""
    content = parsed_asset.get("content", "")
    metadata = parsed_asset.get("metadata", {})
    title = parsed_asset.get("title", "未命名 JSON 资产")
    lines = [f"# {title}", ""]
